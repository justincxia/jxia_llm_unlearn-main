...
def create_pku_dataloader_from_dataset(tokenizer, dataset, fraction=1.0, batch_size=4):
    ...
    def preproccess(examples):
        ...
        for i in range(len(examples["prompt"])):
            ...
            j = (i + 1) % len(examples["prompt"])
            while len(response_list) == 0:
                if not examples["is_response_0_safe"][j]:
                    response_list.append(examples["response_0"][j])
                if not examples["is_response_1_safe"][j]:
                    response_list.append(examples["response_1"][j])
                j = (j + 1) % len(examples["prompt"])
        ...
        if len(results["input_ids"]) > len(examples["prompt"]):
            results["input_ids"] = results["input_ids"][:len(examples["prompt"])]
            results["attention_mask"] = results["attention_mask"][:len(examples["prompt"])]
            results["start_locs"] = results["start_locs"][:len(examples["prompt"])]
